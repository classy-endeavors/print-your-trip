...
async function checkMinIOReady() {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      console.log(`Checking MinIO connection (attempt ${i + 1}/${maxRetries})...`);
      ...
      console.log(`❌ MinIO not ready yet (attempt ${i + 1}/${maxRetries}): ${error.message}`);
      if (i < maxRetries - 1) {
...
app.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    console.log('File received:', {
      ...
      size: req.file.size,
      buffer: req.file.buffer ? `Buffer(${req.file.buffer.length})` : 'No buffer'
    });
